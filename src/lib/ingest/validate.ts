...
/**
 * PDFs are rasterized in the browser, so the server never parses one. What
 * arrives is checked on content, not the declared type: a caller can claim any
 * MIME type it likes.
 */
...
/** Identifies by leading bytes, ignoring whatever the caller declared. */
...
/**
 * Throws a `ValidationError` whose message is safe to show the user.
 *
 * The pixel cap guards against decompression bombs: a small file can declare
 * enormous dimensions, and every page gets base64-encoded into a model request.
 */
